...
\newpage
 ...
 化学式：\hl{\ce{Al2CO3}}・\hl{\ce{Al(OH)3}}
 酸化アルミニウムの別称：\hl{アルミナ}
 \subsubsection{性質}
 \begin{itemize}
  \item \hl{白}色で、水に\hl{溶けにくい}\\
 ...
 \subsubsection{製法}
 \begin{itemize}
  \item 錫石\ce{SnO2}にコークスを混ぜて加熱 \K\\
  \hce{SnO2 + 2C -> Sn + 2CO}
  \item 方鉛鉱\ce{PbS}を焙焼してから、コークスを混ぜて加熱 \K\\
...
\end{tikzpicture}
 ...
 \end{tabular}
 \begin{itemize}
 \item \ce{Fe^2+,Fe^3+}は、\hl{\ce{OH-}}とも\hl{\ce{OH-}}とも錯イオンを形成しない
 ...
 \begin{itemize}
  ...
  \hl{赤}色の酸化銅（\ajRoman{1}）\stamp{Cyan}{乾}・\hl{青緑}の錆（\hl{緑青}）\stamp{RoyalBlue}{湿}
  \item 空気中で加熱して、\hl{黒}色の\hl{酸化銅（\ajRoman{2}）}（$1000^\circ$C未満）・\hl{赤}色の\hl{酸化銅（\ajRoman{1}）}（$1000^\circ$C以上）
 \end{itemize}
 \subsubsection{製法}
...
\draw[->,thick] (from.east)--(to1.west);
\draw[->,thick] (from.east)--(to2.west);
\end{tikzpicture}
\end{quotation}
\begin{tikzpicture}[remember picture, baseline=(tenro1.base)]
 \node[draw=black, rectangle, rounded corners=2pt, line width=1pt, inner sep=5pt, outer sep=0pt, right] (tenro1) {\hl{転炉}};
 \draw[-latex,very thick,overlay] (kouro1.south)--(tenro1.north);
\end{tikzpicture}
