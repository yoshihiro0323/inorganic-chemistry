...
\subsubsection{製法}
\begin{itemize}
      \item 酸化銀（\ajRoman{1}）にフッ化水素酸を加えて蒸発圧縮 \stamp{teal}{フッ化銀}\\
...
\section{酸素}
\subsection{酸素原子}
同\hl{素}体：酸素(\ce{O2}),\hl{オゾン}(\ce{O3})\\
...
\begin{itemize}
      ...
      \item \hl{還元}剤としての反応\\
            \hce{H2S -> S + 2H+ + 2e-}
      \item 重金属イオン\ce{M^2+}と\hl{難溶性の塩}を生成\\
            \hce{M2+ + S^{2-} <=> MS v}
\end{itemize}
\subsubsection{製法}
\begin{itemize}
...
\begin{itemize}
      \item 硫化水素とヨウ素\\
            \hce{H2S + I2 -> S + 2HI}
      \item \hl{酢酸鉛（\ajRoman{2}）}水溶液と硫化水素（\ce{H2S}の検出）\\
            \hce{(CH3COO)2Pb + H2S -> 2CH3COOH + PbS v}
\end{itemize}
\subsection{二酸化硫黄（亜硫酸ガス）}
化学式：\hl{\ce{SO2}}
電子式：\quad\hlbox{\chemfig{\charge{[circle]0=\:,90=\:,180=\:,270=\:}{O}-[,0.5,,,opacity=0]\charge{[circle]0=\:,-90=\:}{S}-[,0.6,,,opacity=0]\charge{[circle]90=\:,180=\:,270=\:}{O}}}
\subsubsection{性質}
\begin{itemize}
...
\subsubsection{製法}
\begin{itemize}
      \item 硫黄や硫化物の\hl{燃焼} \stamp{teal}{硫化水素} \K\\
...
\begin{itemize}
      \item \hl{ハーバーボッシュ法} \K\\
            \hl{高}温\hl{高}圧で、\hl{四酸化三鉄}（\hl{\ce{Fe3O4}}）触媒\\
            \hce{N2 + 3H2 <=> 2NH3}
...
\begin{itemize}
      ...
      \item \hl{\ce{Al}},\hl{\ce{Cr}},\hl{\ce{Fe}},\hl{\ce{Co}},\hl{\ce{Ni}}は\hl{酸化皮膜}が生じて不溶 \stamp{purple}{濃硝酸}$\Rightarrow$\hl{不動態}
      \item \hl{王水}（\hl{濃塩酸}:\hl{濃硝酸}=3:1）は、\ce{Pt},\ce{Au}も溶解
...
\begin{tabular}{|c||c|c|c|}\hline
      ...
      \chemfig{-[,0.6,,,black,line width=0.5pt,dash pattern=on 2pt off 1.4pt]P(-[:45,0.8]P?[a])(-[:-30,0.8]P?[a]?[b])-[,,,,white]P?[a]?[b]-[,0.6]P(-[:45,0.8]P?[c])(-[:-30,0.8]P?[c]?[d])-[,,,,white]P?[c]?[d]-[,0.6,,,black,line width=0.5pt,dash pattern=on 2pt off 1.4pt]} & 略                                                                               \\ \hline
...
\begin{itemize}
      ...
      \item 赤血球のヘモグロビンの\hl{\ce{Fe^2+}}に対して
      \hfill 強い\hl{酸化結合}
...
\begin{itemize}
      ...
      \item \hl{ケイ酸}を高温（$600^{\circ}$C）で熱分解\\
            \hce{H2SiO3 ->[][\Delta] SiO2 + H2O} $(0<n<1)$
\end{itemize}
\newpage
{\Large \gtfamily シリカゲル生成過程での構造変化}
\definesubmol\omitline{-[::0,0.8,,,dash dot]}
\definesubmol{ONa}{O^{-}Na^{+}}
\begin{multicols*}{2}
      \begin{enumerate}
